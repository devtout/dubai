...
def upload_to_foto(instance, name):
    ...
    return os.path.join('publicidade','fotos', '%s%s'%(horario, extensao))
...
class News(models.Model):
    ...
    def get_absolute_url(self):
        return reverse('news.views.noticia', kwargs={'slug': self.slug})

    def __unicode__(self):
        return self.titulo

def foto_post_save(signal, instance, sender, **kwargs):
    arq = News.objects.get(foto=instance.foto)
